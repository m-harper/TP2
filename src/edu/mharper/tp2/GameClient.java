...
public class GameClient {
	...
	void beginWait() {
		...
		public void run() {
			for ( ; ; ) {
				try {
					if ( c_sockInput.available() > 2) {
						byte buf[] = new byte[1024];
						String command = readCmd(buf);
						//System.out.println("Read in " + readCmd(buf));
						parse(command);
					}
				} catch (IOException e) {
	...
	void parse(String command) {
		System.out.println("About to parse " + command);
		
		// Check if the command is move
		if (command.startsWith("A") || command.startsWith("W")) {
